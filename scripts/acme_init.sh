...
if [[ "${INIT_SYNOLOGY:-}" == "TRUE" ]]; then
  ...
  bash "${BASH_SOURCE%/*}/acme_run.sh" --issue --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" ${ADDITIONAL_ARGUMENTS:-}
  bash "${BASH_SOURCE%/*}/acme_run.sh" --deploy --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --deploy-hook synology_dsm --insecure
...
if [[ "${INIT_FRITZ_BOX:-}" == "TRUE" ]]; then
  ...
  # shellcheck disable=SC2086
  bash "${BASH_SOURCE%/*}/acme_run.sh" --issue --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" --keylength 4096 ${ADDITIONAL_ARGUMENTS:-}
  bash "${BASH_SOURCE%/*}/acme_run.sh" --deploy --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --deploy-hook fritzbox --insecure
...
if [[ "${INIT_UPTIME_KUMA:-}" == "TRUE" ]]; then
  ...
  # shellcheck disable=SC2086
  bash "${BASH_SOURCE%/*}/acme_run.sh" --issue --domain "${CERT_DOMAIN_1}" --domain "${CERT_DOMAIN_2}" --domain "${CERT_DOMAIN_3}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" ${ADDITIONAL_ARGUMENTS:-} \
...
if [[ "${INIT_HOMEBRIDGE:-}" == "TRUE" ]]; then
  ...
  # shellcheck disable=SC2086
  bash "${BASH_SOURCE%/*}/acme_run.sh" --issue --domain "${CERT_DOMAIN}" --dns "${CERT_DNS}" --server "${ACME_DIRECTORY_RESOURCE}" ${ADDITIONAL_ARGUMENTS:-}
